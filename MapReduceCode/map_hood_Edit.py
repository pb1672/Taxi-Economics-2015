    ...
    for values in parseInput():
	    ...
	    passenger_count = values[2]
            ...
            if (pickup_neighborhood!=-1) and (dropoff_neighborhood!=-1):
               print '%s\t%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s' % (pickup_hood+'^'+dropoff_hood,pickup_datetime,month,day,hour,dropoff_datetime,passenger_count,trip_time_in_secs,trip_distance,\
                                                                fare_amount, tip_amount, total_amount, tip_percentage,pickup_hood,dropoff_hood)
        except:
            pass
